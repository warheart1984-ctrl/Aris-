...
from .bridge_intelligence import BridgeIntelligenceEngine
...
def default_desktop_data_root(
    ...
    *,
    ...
    override = str(os.getenv("ARIS_RUNTIME_DESKTOP_ROOT", "")).strip()
    if override:
...
def _workspace_task_status(raw_status: object) -> str:
    ...
    if normalized in {"running", "leased"}:
        return "Running"
    if normalized in {"queued", "pending"}:
        return "Pending"
    if normalized in {"approved", "completed", "done"}:
        return "Done"
    if normalized in {"ready_for_approval", "needs_changes", "blocked", "failed", "review", "error", "cancelled", "interrupted"}:
        return "Blocked"
    return "Blocked"
...
def _workspace_task_priority(raw_status: object) -> str:
    normalized = str(raw_status or "").strip().lower()
    if normalized in {"blocked", "failed", "interrupted", "cancelled"}:
        return "P1"
    if normalized in {"ready_for_approval", "needs_changes"}:
        return "P1"
    if normalized in {"running", "queued", "leased"}:
        return "P2"
    return "P2"


def _agent_run_status(run: dict[str, Any]) -> str:
    blocked_on_approval_id = str(run.get("blocked_on_approval_id", "")).strip()
    if blocked_on_approval_id:
        return "Blocked"
    return _workspace_task_status(run.get("status", ""))


def _agent_run_priority(run: dict[str, Any]) -> str:
    blocked_on_approval_id = str(run.get("blocked_on_approval_id", "")).strip()
    if blocked_on_approval_id:
        return "P1"
    return _workspace_task_priority(run.get("status", ""))


def _orchestrated_task_status(raw_status: object) -> str:
    normalized = str(raw_status or "").strip().lower()
    if normalized == "running":
        return "Running"
    if normalized == "pending":
        return "Pending"
    if normalized in {"blocked", "error", "rejected"}:
        return "Blocked"
    if normalized == "done":
        return "Done"
    return "Pending"
...
def _task_sort_key(task: dict[str, Any]) -> tuple[int, int, str, str]:
    ...
    queue_name = str(task.get("queue_name", OPERATOR_QUEUE)).strip().upper()
    queue_rank = 1 if queue_name == SELF_IMPROVE_QUEUE else 0
    priority = str(task.get("priority", "P2")).strip().upper()
    priority_rank = 99
    ...
    return (
        ...
        queue_rank,
        f"{priority_rank:02d}",
        str(task.get("updated_at", "") or task.get("created_at", "") or task.get("id", "")),
    )


def _agent_run_latest_update(run: dict[str, Any]) -> str:
    final_message = str(run.get("final_message", "")).strip()
    if final_message:
        return final_message
    error_text = str(run.get("error_text", "")).strip()
    if error_text:
        return error_text
    blocked_on_approval_id = str(run.get("blocked_on_approval_id", "")).strip()
    if blocked_on_approval_id:
        blocked_on_kind = str(run.get("blocked_on_kind", "approval")).strip() or "approval"
        return f"Blocked on {blocked_on_kind} {blocked_on_approval_id}. Resume requires operator review."
    status = str(run.get("status", "")).strip().lower()
    if status in {"queued", "leased"}:
        return "Queued in the governed worker lane."
    if status == "running":
        return "ARIS is executing the task through the governed worker lane."
    if status == "completed":
        return "Task completed and is ready for review."
    if status in {"cancelled", "interrupted"}:
        return "Task was stopped before completion."
    if status == "failed":
        return "Task failed and needs operator attention."
    return "ARIS is tracking this run."


def clean_operator_text(text: object) -> str:
    ...
    replacements = (
        ("Forge Eval", "Review"),
        ("ForgeEval", "Review"),
        ("Forge Worker Lane", "Worker Lane"),
        ("Forge Worker", "Worker"),
        ("Forge", "Worker"),
        ("UL Runtime", "Engine"),
        ("UL runtime", "Engine"),
        ("UL Engine", "Engine"),
        ("Evolving Runtime", "Protected Path"),
        ("Evolving Engine", "Protected Path"),
        ("evolving-runtime", "protected"),
        ("evolving engine", "protected path"),
        ("Evolving Core", "Protected Path"),
        ("evolving core", "protected path"),
        ("Router", "Mode"),
        ("router", "mode"),
    )
    ...
    cleaned = re.sub(r"\bJarvis\s*->\s*Operator\s*->\s*Worker\s*->\s*Review\s*->\s*Engine\s*->\s*Outcome\b", "Execution path available in Inspect.", cleaned)
    cleaned = re.sub(r"\s+", " ", cleaned).strip()
    return cleaned


def select_active_task(tasks: list[dict[str, Any]]) -> dict[str, Any] | None:
    running = [task for task in tasks if str(task.get("status", "")).strip() == "Running"]
    if running:
        return sorted(
            running,
            key=lambda task: str(task.get("updated_at", "") or task.get("created_at", "") or task.get("id", "")),
        )[-1]
    pending = [task for task in tasks if str(task.get("status", "")).strip() == "Pending"]
    if pending:
        return sorted(
            pending,
            key=lambda task: str(task.get("created_at", "") or task.get("id", "")),
        )[0]
...
def _extract_diff_for_path(diff_text: str, path: str) -> str:
    ...
    for index, block in enumerate(blocks):
        current = block if index == 0 else f"diff --git {block}"
        if normalized in current:
            matches.append(current.strip())
    excerpt = "\n\n".join(match for match in matches if match).strip()
    if not excerpt:
        return ""
    if len(excerpt) > 4000:
        excerpt = f"{excerpt[:4000]}\n...[diff excerpt truncated]..."
    return excerpt


def _operator_event_message(event: dict[str, Any]) -> str:
    if not isinstance(event, dict):
        return ""
    kind = str(event.get("kind", "")).strip().lower()
    if kind == "approval_required":
        return "Review is required before changes can continue."
    if kind == "approval_decision":
        approved = bool(event.get("approved"))
        return "Changes were approved for continuation." if approved else "Changes were rejected and kept contained."
    for field in ("summary", "detail", "message", "content", "title"):
        text = clean_operator_text(event.get(field, ""))
        if text:
            return text
    return ""


def build_passive_review_payload(workspace_payload: dict[str, Any] | None) -> dict[str, Any]:
    workspace = _dict(workspace_payload)
    pending_patches = (
        list(workspace.get("pending_patches", []))
        if isinstance(workspace.get("pending_patches", []), list)
        else []
    )
    applied_changes = (
        list(workspace.get("applied_changes", []))
        if isinstance(workspace.get("applied_changes", []), list)
        else []
    )
    ...
    if applied_changes:
        summary_lines.append(f"Applied workspace edits: {len(applied_changes)} change(s) tracked locally.")
        for change in applied_changes[:8]:
    ...
    git_changed = git.get("changed_files", [])
    if isinstance(git_changed, list):
        for item in git_changed[:20]:
            path = str(item).strip()
            if path:
                changed_files.append(path)
                changed_entries.append(
    ...
    git_diff_stat = str(git.get("diff_stat", "")).strip()
    if git_diff_stat:
        summary_lines.append(git_diff_stat)
    git_diff = str(git.get("diff", "")).strip()
    if git_diff:
        diff_sections.append(git_diff)

    if not summary_lines:
        summary_lines.append("No workspace changes are waiting right now.")
    ...
    for entry in changed_entries:
        key = (str(entry.get("path", "")).strip(), str(entry.get("status", "")).strip())
    ...
    return {
        "ok": True,
        "summary": "\n".join(summary_lines),
        "changed_files": sorted({item for item in changed_files if item}),
        "changed_entries": deduped_entries,
...
def build_operator_bridge(
    *,
    ...
    active_task = select_active_task(tasks)
    review = _dict(review_payload)
    router = _dict(model_router)
    run = _dict(run_payload)
    events = [item for item in list(run_events or []) if isinstance(item, dict)]
    repo_lookup = {
        str(repo.get("id", "")).strip(): str(repo.get("name", "Workspace")).strip() or "Workspace"
        for repo in repos
        if isinstance(repo, dict)
    }
    repo_name = repo_lookup.get(str(_dict(active_task).get("repo_id", "")).strip(), "Workspace")
    task_title = str(_dict(active_task).get("title", "No active task")).strip() or "No active task"
    task_status = str(_dict(active_task).get("status", "Idle")).strip() or "Idle"
    router_mode = str(router.get("mode", "auto")).strip().title() or "Auto"

    stream_lines: list[str] = []
    if events:
        for event in events[-8:]:
            message = _operator_event_message(event)
            if not message:
                continue
            timestamp = _display_time(event.get("created_at", "") or event.get("timestamp", "") or event.get("time", ""))
            stream_lines.append(f"[{timestamp}] {message}")
    latest_update = clean_operator_text(_dict(active_task).get("latest_update", ""))
    if latest_update and not any(latest_update in line for line in stream_lines):
        timestamp = _display_time(_dict(active_task).get("updated_at", "") or _dict(active_task).get("created_at", ""))
        stream_lines.append(f"[{timestamp}] {latest_update}")
    approval_id = str(_dict(active_task).get("approval_id", "")).strip()
    review_gate = str(_dict(active_task).get("review_gate", "")).strip().lower()
    if approval_id or review_gate == "operator_review":
        stream_lines.append("[Now] Changes are waiting for review before apply.")
    changed_entries = [
        item
        for item in list(review.get("changed_entries", []))
        if isinstance(item, dict) and str(item.get("path", "")).strip()
    ]
    if changed_entries:
        stream_lines.append(f"[Now] {len(changed_entries)} changed file(s) are available to inspect.")
    if not stream_lines:
        stream_lines.append("[Now] No governed run activity is available yet.")

    diff_text = str(review.get("diff", "")).strip()
    change_items: list[dict[str, str]] = []
    seen_paths: set[str] = set()
    for entry in changed_entries[:12]:
        path = str(entry.get("path", "")).strip()
        if not path or path in seen_paths:
            continue
        seen_paths.add(path)
        summary = clean_operator_text(entry.get("summary", "")) or clean_operator_text(path)
        change_items.append(
            {
                "path": path,
                "status": str(entry.get("status", "")).strip() or "?",
                "summary": summary,
                "diff": _extract_diff_for_path(diff_text, path),
            }
        )
    if not change_items and diff_text:
        change_items.append(
            {
                "path": "workspace.diff",
                "status": "D",
                "summary": "Workspace diff is available to inspect.",
                "diff": diff_text[:4000] + ("\n...[diff excerpt truncated]..." if len(diff_text) > 4000 else ""),
            }
        )

    return {
        "header_title": f"{repo_name} / {task_title}",
        "header_status": f"Status: {task_status}",
        "header_mode": f"Mode: {router_mode}",
        "stream_lines": stream_lines,
        "changes_title": "Changes",
        "changes_summary": clean_operator_text(review.get("summary", "")) or "No workspace changes are waiting right now.",
        "changes": change_items,
        "active_task_id": str(_dict(active_task).get("id", "")).strip(),
...
def build_workspace_surface(
    *,
    ...
    approval_audit: tuple[dict[str, Any], ...] = (),
    orchestrated_tasks: tuple[dict[str, Any], ...] = (),
    status_payload: dict[str, Any] | None = None,
) -> dict[str, Any]:
    ...
    if orchestrated_tasks:
        for item in orchestrated_tasks:
            ...
            linked_run_id = str(item.get("linked_run_id", "")).strip()
            if linked_run_id:
                linked_run_ids.add(linked_run_id)
            queue_name = str(item.get("queue_name", OPERATOR_QUEUE)).strip().upper() or OPERATOR_QUEUE
            requires_approval = bool(item.get("requires_approval", False))
            tasks.append(
                {
                    "id": task_id,
                    "title": str(item.get("title", "")).strip() or "Queued task",
                    "repo_id": primary_repo_id,
                    "status": _orchestrated_task_status(item.get("status", "")),
                    "priority": _priority_label(item.get("priority", 2)),
                    "latest_update": str(item.get("latest_update", "")).strip() or "Queued for governed execution.",
                    "summary": str(item.get("prompt", "")).strip() or str(item.get("title", "")).strip(),
                    "raw_status": str(item.get("status", "")).strip() or "pending",
                    "task_type": "orchestrated",
                    "run_id": linked_run_id,
                    "linked_run_id": linked_run_id,
                    "approval_id": str(item.get("approval_id", "")).strip(),
                    "review_gate": str(item.get("review_gate", "")).strip(),
                    "queue_name": queue_name,
                    "depends_on": [str(entry).strip() for entry in list(item.get("depends_on", [])) if str(entry).strip()],
    ...
    if agent_runs:
        for run in agent_runs:
            if not isinstance(run, dict):
                continue
            run_id = str(run.get("id", "")).strip()
            if not run_id or run_id in seen_task_ids or run_id in linked_run_ids:
                continue
            seen_task_ids.add(run_id)
            title = (
                str(run.get("title", "")).strip()
                or str(run.get("user_message", "")).strip()
                or "Agent run"
            )
            summary = str(run.get("user_message", "")).strip() or title
            blocked_on_approval_id = str(run.get("blocked_on_approval_id", "")).strip()
            tasks.append(
                {
                    "id": run_id,
                    ...
                    "priority": _agent_run_priority(run),
                    "latest_update": _agent_run_latest_update(run),
                    "summary": summary,
                    "raw_status": str(run.get("status", "")).strip() or "unknown",
                    "task_type": "agent_run",
                    "run_id": run_id,
                    "approval_id": blocked_on_approval_id,
                    "review_gate": "runtime_approval" if blocked_on_approval_id else "",
                    "queue_name": OPERATOR_QUEUE,
                    "depends_on": [],
                    "blocked_on_kind": str(run.get("blocked_on_kind", "")).strip(),
    ...
    if task_entries:
        for index, item in enumerate(task_entries):
            if not isinstance(item, dict):
                continue
            task_id = str(item.get("id", f"task-{index + 1}")).strip() or f"task-{index + 1}"
    ...
    if not [
        task
        for task in tasks
        if str(task.get("queue_name", OPERATOR_QUEUE)).strip().upper() != SELF_IMPROVE_QUEUE
    ]:
        fallback_tasks = (
            (
            ...
            (
                ...
                "Verify that no evolving-core path is visible, routable, or callable from the workspace.",
            ),
        )
    ...
    active_orchestrated = next(
        (
            item
            for item in orchestrated_tasks
            if isinstance(item, dict)
            and str(item.get("status", "")).strip().lower() in ACTIVE_QUEUE_STATUSES
            and (
                str(item.get("queue_name", OPERATOR_QUEUE)).strip().upper() != SELF_IMPROVE_QUEUE
                or str(item.get("linked_run_id", "")).strip()
            )
        ),
        {},
    )
    ...
    elif active_orchestrated:
        worker_status = str(active_orchestrated.get("status", "pending")).strip().title() or "Pending"
        worker_title = "Governed Queue"
        worker_lines = [
            f"{active_orchestrated.get('title', 'Task')} is staged in the {str(active_orchestrated.get('queue_name', OPERATOR_QUEUE)).strip().replace('_', ' ').title()} lane.",
            str(active_orchestrated.get("latest_update", "")).strip() or "Queued in the governed worker lane.",
            f"Priority {int(active_orchestrated.get('priority', 2) or 2)} with {len(list(active_orchestrated.get('depends_on', [])))} dependency link(s).",
        ]
    elif agent_runs:
        latest_run = next((item for item in agent_runs if isinstance(item, dict)), {})
        latest_run_id = str(latest_run.get("id", "")).strip()
        latest_status = str(latest_run.get("status", "queued")).strip() or "queued"
        worker_status = _agent_run_status(latest_run)
        worker_title = "Governed Agent Lane"
        worker_lines = [
            f"Run {latest_run_id or 'pending'} is {latest_status}.",
            str(latest_run.get("user_message", "")).strip() or "ARIS is handling the active task.",
            _agent_run_latest_update(latest_run),
        ]
    ...
    return {
        ...
        "agent_runs": [dict(item) for item in agent_runs if isinstance(item, dict)],
        "approval_audit": [dict(item) for item in approval_audit if isinstance(item, dict)],
...
def build_feature_inventory(
    *,
    ...
    runtime_mode = _dict(status_payload.get("runtime_mode") or status_payload.get("demo_mode"))
    ...
    features = [
        ...
        DesktopFeature(
            ...
            status=(
                ...
                else ("stripped" if runtime_mode.get("active") else "offline")
            ),
            ...
            detail=(
                ...
                if forge.get("connected")
                    ...
                    "Forge and Forge Eval stay truthfully surfaced as stripped in this runtime profile."
                    if runtime_mode.get("active")
        ...
        DesktopFeature(
            ...
            detail=(
                ...
                if evolving_engine.get("active")
                    ...
                    evolving_engine.get(
                        "reason",
                        "The evolving engine remains outside this ARIS runtime profile.",
                    )
                )
            ),
        ),
...
class ArisRuntimeDesktopHost:
    """Desktop host wrapper over the existing ARIS V2 service."""
    ...
    def __init__(
        ...
        *,
        ...
        self.data_root = self._ensure_data_root(
            ...
            fallback_name=f"aris_runtime_desktop_{self.profile.id}",
        )
        self.config = AppConfig.from_env(self.data_root)
        self.service = ArisRuntimeChatService(self.config, profile_id=self.profile.id)
        self.app_version = str(os.getenv("ARIS_RUNTIME_APP_VERSION", "0.1.0")).strip() or "0.1.0"
        ...
        self._self_improve_history_path = (self.data_root / "self-improve-history.jsonl").resolve()
        self.operator_queue = OperatorQueueStore((self.data_root / "operator-queue.json").resolve())
        self.workspace_registry = WorkspaceRegistry(
            (self.data_root / "workspace_registry.json").resolve(),
            seed_root=Path(__file__).resolve().parents[2],
        )
        memory_bank = getattr(getattr(self.service, "aris", None), "memory_bank", None)
        self.bridge_intelligence = BridgeIntelligenceEngine(
            (self.data_root / "bridge_intelligence").resolve(),
            memory_bank=memory_bank,
        )
        self._workers_started = False
        self._scheduler_stop = threading.Event()
        self._scheduler_thread: threading.Thread | None = None
        self._scheduler_max_concurrency = 2
    ...
    def create_session(self, title_seed: str | None = None) -> str:
        ...
        self.operator_queue.ensure_self_improve_seed(session_id=session.id)
        return session.id
    ...
    def ensure_session(self, session_id: str | None = None, *, title_seed: str | None = None) -> str:
        if session_id:
            ...
            if existing is not None:
                self.operator_queue.ensure_self_improve_seed(session_id=existing.id)
                return existing.id
        sessions = self.service.list_sessions()
        if sessions:
            resolved = str(sessions[0].get("id", "")).strip() or self.create_session(title_seed)
            self.operator_queue.ensure_self_improve_seed(session_id=resolved)
            return resolved
    ...
    def _ensure_scheduler_started(self) -> None:
        ...
        self._scheduler_thread = threading.Thread(
            ...
            name="aris-runtime-operator-scheduler",
            daemon=True,
        )
    ...
    def list_operator_queue(self, *, session_id: str | None = None) -> list[dict[str, Any]]:
        return self.operator_queue.list_items(session_id=session_id)

    def enqueue_operator_task(
        self,
        *,
        session_id: str | None,
        title: str,
        ...
        queue_name: str = OPERATOR_QUEUE,
        requires_approval: bool = False,
        source: str = "operator",
        metadata: dict[str, Any] | None = None,
    ) -> dict[str, Any]:
        resolved_session_id = self.ensure_session(session_id, title_seed=title)
        item = self.operator_queue.enqueue(
        ...
        self._record_event(
            "scheduler",
            "Task queued",
            f"{item.get('title', 'Task')} entered the {item.get('queue_name', OPERATOR_QUEUE)} lane at {item.get('priority', 2)}.",
            workspace_id=str(self.active_workspace().get("id", "")),
        )
        return item

    def resolve_operator_review(
        self,
        *,
        task_id: str,
        approved: bool,
        ...
        item = self.operator_queue.get_item(task_id)
        if item is None:
            return None
        latest_update = (
            "Operator approved the self-improvement result for adoption."
            if approved
            else "Operator rejected the self-improvement result and kept it out of admission."
        )
        updated = self.operator_queue.update_item(
            task_id,
        ...
        if updated is not None:
            self._record_self_improve_outcome(updated)
            self._record_event(
                "scheduler",
                "Self-improve review resolved",
                latest_update,
                status="ok" if approved else "fail",
                workspace_id=str(self.active_workspace().get("id", "")),
            )
        return self.operator_queue.get_item(task_id) if updated is not None else None

    def task_memory(self, task_id: str, *, title: str = "") -> dict[str, Any]:
        return self.bridge_intelligence.task_memory.get(task_id, title=title)
    ...
    def save_task_memory(
        ...
        *,
        ...
        record = self.bridge_intelligence.save_task_memory(
            task_id=task_id,
            title=title,
            goals=goals,
        ...
        self._record_event(
            "memory",
            "Task memory updated",
            f"Structured task memory for {record.get('title', task_id)} was saved.",
            workspace_id=str(self.active_workspace().get("id", "")),
        )
        return record
    ...
    def record_rejection_reason(
        ...
        *,
        ...
        record = self.bridge_intelligence.record_rejection(
            task_id=task_id,
            title=title,
            reason=reason,
            note=note,
        ...
        self._record_event(
            ...
            f"{title or task_id} was rejected because: {reason}.",
            status="fail",
            workspace_id=str(self.active_workspace().get("id", "")),
        )
        return record
    ...
    def _dependency_ids_pending(
        ...
        pending: list[str] = []
        for dependency_id in [str(entry).strip() for entry in list(item.get("depends_on", [])) if str(entry).strip()]:
            dependency = items_by_id.get(dependency_id)
            dependency_status = str((dependency or {}).get("status", "")).strip().lower()
    ...
    def _sync_operator_task(
        ...
        *,
        ...
        if not item_id:
            return None
        current_status = str(item.get("status", "")).strip().lower() or "pending"
        updates: dict[str, Any] = {}
        dependency_ids = self._dependency_ids_pending(item, items_by_id)
        linked_run_id = str(item.get("linked_run_id", "")).strip()
        if not linked_run_id:
            if current_status not in TERMINAL_QUEUE_STATUSES:
                if dependency_ids:
                    updates["status"] = "blocked"
                    updates["latest_update"] = f"Waiting on {len(dependency_ids)} dependency task(s) before launch."
                elif current_status == "blocked":
                    updates["status"] = "pending"
                    updates["latest_update"] = "Ready for governed execution."
            return self.operator_queue.update_item(item_id, **updates) if updates else None

        run = agent_runs.get(linked_run_id)
        if run is None:
            return None
        blocked_on_approval_id = str(run.get("blocked_on_approval_id", "")).strip()
        run_status = str(run.get("status", "")).strip().lower()
        latest_update = _agent_run_latest_update(run)
        updates["approval_id"] = blocked_on_approval_id
        if blocked_on_approval_id or run_status == "blocked":
            if str(item.get("queue_name", "")).strip().upper() == SELF_IMPROVE_QUEUE and not blocked_on_approval_id:
                updates["review_gate"] = "operator_review"
            else:
                updates["review_gate"] = "runtime_approval"
            updates["status"] = "blocked"
            updates["latest_update"] = latest_update
        elif run_status in {"queued"}:
            updates["status"] = "pending"
            updates["latest_update"] = latest_update
            updates["review_gate"] = ""
        elif run_status in {"running", "leased"}:
            updates["status"] = "running"
            updates["latest_update"] = latest_update
            updates["review_gate"] = ""
        elif run_status == "completed":
            if bool(item.get("requires_approval")) and not bool(item.get("approved")) and not bool(item.get("rejected")):
                updates["status"] = "blocked"
                updates["review_gate"] = "operator_review"
                updates["latest_update"] = (
                    "Self-improvement result is ready for operator approval before merge."
                )
            else:
                updates["status"] = "done"
                updates["review_gate"] = ""
                updates["latest_update"] = latest_update or "Task completed."
                updates["completed_at"] = str(run.get("completed_at", "")).strip() or _utc_now()
        elif run_status in {"failed", "interrupted", "cancelled"}:
            updates["status"] = "error"
            updates["review_gate"] = ""
            updates["latest_update"] = latest_update
            updates["completed_at"] = str(run.get("completed_at", "")).strip() or _utc_now()
        if updates:
            return self.operator_queue.update_item(item_id, **updates)
        return None

    def _record_self_improve_outcome(self, item: dict[str, Any]) -> None:
        if str(item.get("queue_name", "")).strip().upper() != SELF_IMPROVE_QUEUE:
            return
        if bool(item.get("outcome_recorded")):
            return
        payload = {
            "task_id": str(item.get("id", "")).strip(),
            "linked_run_id": str(item.get("linked_run_id", "")).strip(),
        ...
        self._self_improve_history_path.parent.mkdir(parents=True, exist_ok=True)
        with self._self_improve_history_path.open("a", encoding="utf-8") as handle:
            handle.write(json.dumps(payload, ensure_ascii=True) + "\n")
        if self.bridge_intelligence.memory_bank is not None:
            content = json.dumps(payload, ensure_ascii=True)
            if bool(item.get("approved", False)):
                self.bridge_intelligence.memory_bank.admit_learned_pattern(
                    name="self_improve_outcome",
                    summary=f"Approved self-improvement: {str(item.get('title', 'Task')).strip()}",
                    content=content,
                    source="aris_runtime_desktop",
                    tags=("self-improve", _slug(item.get("id", "")), "approved"),
                )
            else:
                self.bridge_intelligence.memory_bank.reject_pattern(
                    name="self_improve_outcome",
                    summary=f"Rejected self-improvement: {str(item.get('title', 'Task')).strip()}",
                    content=content,
                    source="aris_runtime_desktop",
                    tags=("self-improve", _slug(item.get("id", "")), "rejected"),
                )
        self.operator_queue.update_item(str(item.get("id", "")).strip(), outcome_recorded=True)
    ...
    def scheduler_tick(
        ...
        *,
        ...
        with self._scheduler_lock:
            bounded_concurrency = max(1, int(max_concurrency or self._scheduler_max_concurrency))
            sessions = (
                ...
                if session_id
                else sorted(
                    {
                        ...
                        for item in self.operator_queue.list_items()
            ...
            for current_session_id in sessions:
                self.operator_queue.ensure_self_improve_seed(session_id=current_session_id)
                items = self.operator_queue.list_items(session_id=current_session_id)
                items_by_id = {
                ...
                agent_runs = {
                    str(run.get("id", "")).strip(): dict(run)
                    for run in self.list_agent_runs(current_session_id, limit=120)
                    if isinstance(run, dict) and str(run.get("id", "")).strip()
                }
                for item in items:
                    synced = self._sync_operator_task(
                        item,
                        items_by_id=items_by_id,
                        agent_runs=agent_runs,
                    )
                    if synced is not None:
                        items_by_id[str(synced.get("id", "")).strip()] = dict(synced)
                items = self.operator_queue.list_items(session_id=current_session_id)
                for item in items:
                    if (
                        str(item.get("queue_name", "")).strip().upper() == SELF_IMPROVE_QUEUE
                        and str(item.get("status", "")).strip().lower() in {"done", "rejected", "error"}
                    ):
                        self._record_self_improve_outcome(item)
                active = [
                    item
                    for item in items
                    if str(item.get("status", "")).strip().lower() in {"pending", "running"}
                    and str(item.get("linked_run_id", "")).strip()
                ]
                slots = max(0, bounded_concurrency - len(active))
                if slots <= 0 or not self._workers_started:
                    continue
                candidates = []
                for item in items:
                    if str(item.get("status", "")).strip().lower() != "pending":
                        continue
                    if str(item.get("linked_run_id", "")).strip():
                        continue
                    if self._dependency_ids_pending(item, items_by_id):
                        continue
                    candidates.append(item)
                candidates.sort(
                    key=lambda item: (
                        -max(1, int(item.get("priority", 1) or 1)),
                        str(item.get("created_at", "")),
                    )
                )
                for item in candidates[:slots]:
                    response = self.service.enqueue_agent_run(
                        session_id=current_session_id,
                        user_message=str(item.get("prompt", "")).strip(),
                        title=str(item.get("title", "")).strip(),
                        fast_mode=False,
                        metadata={
                            "queue_name": str(item.get("queue_name", OPERATOR_QUEUE)).strip().upper(),
                            "priority": int(item.get("priority", 1) or 1),
                            "depends_on": [str(entry).strip() for entry in list(item.get("depends_on", [])) if str(entry).strip()],
                            "source": str(item.get("source", "operator")).strip(),
                        },
                    )
                    run = dict(response.get("run", {})) if isinstance(response, dict) else {}
                    run_id = str(run.get("id", "")).strip()
                    if not run_id:
                        continue
                    updated = self.operator_queue.update_item(
                        ...
                        status="pending",
                        latest_update="Queued in the governed worker lane.",
                        started_at=_utc_now(),
                    )
                    if updated is not None:
                        ...
                        self._record_event(
                            "scheduler",
                            "Task started",
                            f"{updated.get('title', 'Task')} launched into the governed worker lane as run {run_id}.",
                            workspace_id=str(self.active_workspace().get("id", "")),
                        )
    ...
    def snapshot(self, session_id: str | None = None) -> DesktopSnapshot:
        ...
        if active_session_id:
            self.scheduler_tick(session_id=active_session_id, max_concurrency=self._scheduler_max_concurrency)
        ...
        review_payload = build_passive_review_payload(workspace)
        agent_runs = (
            tuple(self.service.list_agent_runs(active_session_id, limit=12).get("runs", []))
            if active_session_id
            else ()
        )
        approval_audit = (
            tuple(self.service.list_approval_audit(active_session_id, limit=20).get("entries", []))
            if active_session_id
            else ()
        )
        orchestrated_tasks = (
            tuple(self.operator_queue.list_items(session_id=active_session_id))
            if active_session_id
            else ()
        )
        ...
        operator_bridge = build_operator_bridge(
            repos=[item for item in list(workspace_surface.get("repos", [])) if isinstance(item, dict)],
            tasks=[item for item in list(workspace_surface.get("tasks", [])) if isinstance(item, dict)],
        ...
        active_run_id = str(operator_bridge.get("run_id", "")).strip()
        if active_run_id:
            try:
                operator_bridge = build_operator_bridge(
                    repos=[item for item in list(workspace_surface.get("repos", [])) if isinstance(item, dict)],
                    ...
                    run_payload=self.get_agent_run(active_run_id),
                    run_events=self.list_agent_run_events(active_run_id, limit=40),
                )
            except Exception:
                pass
        active_task = select_active_task(
            [item for item in list(workspace_surface.get("tasks", [])) if isinstance(item, dict)]
        )
        run_payload = self.get_agent_run(active_run_id) if active_run_id else {}
        run_events = self.list_agent_run_events(active_run_id, limit=40) if active_run_id else []
        bridge_intelligence = self.bridge_intelligence.build_for_task(
            task=active_task,
            review=review_payload,
            run=run_payload,
            run_events=run_events,
            local_events=self.recent_events(limit=20),
        )
        operator_bridge["intelligence"] = bridge_intelligence
        approval_summary = _dict(bridge_intelligence.get("approval_summary"))
        if approval_summary:
            operator_bridge["changes_summary"] = str(
                approval_summary.get("summary", operator_bridge.get("changes_summary", ""))
            ).strip() or operator_bridge.get("changes_summary", "")
        operator_bridge["task_memory"] = bridge_intelligence.get("task_memory", {})
        operator_bridge["replay"] = bridge_intelligence.get("replay", {})
        operator_bridge["branches"] = bridge_intelligence.get("branches", [])
        ...
        workspace_surface.update(
            {
                ...
                "agent_runs": [dict(item) for item in agent_runs if isinstance(item, dict)],
                "orchestrated_tasks": [dict(item) for item in orchestrated_tasks if isinstance(item, dict)],
                "approval_audit": [dict(item) for item in approval_audit if isinstance(item, dict)],
                "review": review_payload,
                "operator_bridge": operator_bridge,
                "bridge_intelligence": bridge_intelligence,
                "task_memory": bridge_intelligence.get("task_memory", {}),
                "replay": bridge_intelligence.get("replay", {}),
                "branches": bridge_intelligence.get("branches", []),
                "upgrades": [
                    {
    ...
    def iter_chat_events(
        ...
        *,
        ...
        if normalized_mode == "agent" and not self._workers_started:
    ...
    def list_agent_runs(self, session_id: str, *, limit: int = 20) -> list[dict[str, Any]]:
        return list(self.service.list_agent_runs(session_id, limit=limit).get("runs", []))

    def get_agent_run(self, run_id: str) -> dict[str, Any]:
        return dict(self.service.get_agent_run(run_id))
    ...
    def list_agent_run_events(
        ...
        *,
        ...
        return list(
            self.service.agent_runs.list_events(
                run_id=run_id,
                after_id=after_id,
                limit=limit,
            )
    ...
    def cancel_agent_run(self, run_id: str) -> dict[str, Any]:
        return dict(self.service.cancel_agent_run(run_id))

    def iter_approval_events(
        self,
        *,
        session_id: str,
        approval_id: str,
        ...
        if not self._workers_started:
            notice = "Approval resolution requires the governed worker lane, which is disabled in this host."
            yield DesktopChatEvent(
                event="meta",
                payload={
                    "session_id": session_id,
                    "mode": "agent_resume",
        ...
        stream = self.service.stream_approval_decision(
            session_id=session_id,
            approval_id=approval_id,
            approved=approved,
        )
