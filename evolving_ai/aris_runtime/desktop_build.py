...
from .profiles import DEFAULT_PROFILE_ID, aris_runtime_profiles, profile_choices, resolve_profile
...
def _profiles_payload() -> list[dict[str, str]]:
    return [
        ...
        for profile in aris_runtime_profiles()
    ]
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build or inspect native packaging targets for ARIS V2 Desktop.")
    parser.add_argument(
        "--variant",
        choices=profile_choices(),
        default=DEFAULT_PROFILE_ID,
        help="Select which ARIS desktop profile to build.",
    )
    parser.add_argument(
        "--print-profiles",
        action="store_true",
        help="Print the supported ARIS desktop profiles as JSON.",
    )
    parser.add_argument(
        "--print-targets",
        action="store_true",
    ...
    parser.add_argument(
        "--build-all-current",
        action="store_true",
        help="Run PyInstaller once for each supported ARIS desktop profile on the current platform.",
    )
    ...
    if args.build_all_current:
        for item in aris_runtime_profiles():
            subprocess.run(
