...
from evolving_ai.aris_runtime.profiles import resolve_profile
from evolving_ai.aris_runtime.shipping_lane import _entrypoint_name, ship_release, shipping_precheck


REPO_ROOT = Path(__file__).resolve().parents[1]
TEST_RUNTIME_ROOT = REPO_ROOT / ".runtime" / "aris-runtime-shipping-test"
...
class ArisRuntimeShippingLaneTests(unittest.TestCase):
    ...
    def test_ship_release_writes_manifest_release_folders_and_zips(self) -> None:
        ...
        def fake_run_command(command: list[str], *, cwd: Path):
            ...
            if "-m" in command and "evolving_ai.aris_runtime.desktop" in command:
        ...
        with patch("evolving_ai.aris_runtime.shipping_lane.shipping_precheck") as precheck_mock, patch(
            "evolving_ai.aris_runtime.shipping_lane.build_pyinstaller_command",
            side_effect=fake_build_command,
        ), patch("evolving_ai.aris_runtime.shipping_lane._run_command", side_effect=fake_run_command):
