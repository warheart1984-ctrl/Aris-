...
try:
    ...
    from evolving_ai.aris_demo.profiles import DEFAULT_PROFILE_ID, profile_choices, resolve_profile
    from evolving_ai.aris_demo.desktop_support import ArisDemoDesktopHost


def _smokecheck_payload(host: ArisDemoDesktopHost) -> dict[str, object]:
    ...
    return {
        ...
        "demo_mode": snapshot.status.get("demo_mode", {}),
    }


def main(argv: list[str] | None = None, *, default_profile: str = DEFAULT_PROFILE_ID) -> int:
    parser = argparse.ArgumentParser(description="Launch the ARIS Demo desktop host.")
    ...
    parser.add_argument(
        ...
        help="Select the ARIS Demo desktop profile to launch.",
    )
    parser.add_argument(
        "--headless-smokecheck",
        action="store_true",
    ...
    try:
        ...
        try:
            ...
            try:
                from evolving_ai.aris_demo.desktop_app import launch_desktop_app
