...
def _status_lines(status: dict[str, object]) -> list[str]:
    ...
    demo_mode = status.get("demo_mode", {}) if isinstance(status.get("demo_mode"), dict) else {}
    evolving_engine = (
    ...
    return [
        f"ARIS Demo startup target: {status.get('repo_target', Path.cwd())}",
        ...
        f"Forge stripped: {demo_mode.get('active', False)}",
        f"Evolving engine stripped: {not evolving_engine.get('active', False)}",
        "Runtime route: " + " -> ".join(str(item) for item in demo_mode.get("route", [])),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Launch the ARIS Demo governed service.")
    parser.add_argument(
        "--healthcheck",
        action="store_true",
        help="Print current ARIS Demo startup state and exit with 0 only when healthy.",
    )
    parser.add_argument(
        "--reseal-integrity",
        action="store_true",
    ...
    if args.reseal_integrity:
        try:
            ...
            print(f"ARIS Demo integrity reseal failed: {exc}", file=sys.stderr)
            raise SystemExit(1) from exc
    status = service.aris_status_payload()
    print("ARIS Demo")
    print("Advanced Repo Intelligence Service Demo")
    ...
    if status.get("startup_blockers"):
        print("ARIS Demo is starting in fail-closed lockdown so health, logs, and hall state remain inspectable.")
    uvicorn.run(
        create_app(),
