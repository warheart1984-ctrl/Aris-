...
def workspace_status_pills(brain_state: Mapping[str, Any]) -> list[str]:
    ...
    approval_pill = "Approval Gated" if brain["permission"] in {"Approval Required", "Suggest Only", "Read Only"} else "Safe Demo Actions"
...
def build_workspace_demo_decision(
    *,
    prompt: str,
    ...
    if protected_request(prompt, brain["target"]):
        return {
            ...
            "content": (
                ...
                "The evolving core is not exposed to ARIS in demo mode. "
    ...
    if brain["target"] == "Forge":
        ...
        if brain["permission"] in {"Approval Required", "Suggest Only", "Read Only"}:
            ...
            task_update = "Forge is preparing the task now under the selected demo controls." if isinstance(task, Mapping) else None
            worker_lines = [
